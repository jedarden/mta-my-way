...
import { requireFormData, requireJson, validateContentType } from "./content-type.js";
...
describe("validateContentType middleware", () => {
  ...
  it("logs security events for invalid content types", async () => {
    ...
    expect(logArg.context?.event).toBe("suspicious_request");
    expect(logArg.context?.details?.pattern).toBe("invalid_content_type");
