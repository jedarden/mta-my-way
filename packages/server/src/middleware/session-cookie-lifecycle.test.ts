...
describe("Session Cookie Lifecycle Tests", () => {
  ...
  describe("Session Refresh and Sliding Window", () => {
    ...
    it("should extend session expiration on refresh", async () => {
      ...
      // Get original session expiration
      const originalSession = await createSession(testApiKey.keyId, TEST_CLIENT_IP, TEST_USER_AGENT);
      ...
      // Verify new session has extended expiration (would require internal access)
      // For now, just verify refresh succeeded
      expect(refreshResult!.sessionId).toBeDefined();
    });
  });
