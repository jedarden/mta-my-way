 ...
 *
 * The briefing text is personalized using morningScores to prioritize the
 * user's most-checked morning favorites. Respects quiet hours — skips if
 ...
 * Uses morningScores to prioritize the user's most-relevant favorites.
...
function buildBriefingPayload(
  ...
  // Sort favorites by morning score (highest first) for prioritized briefing
  const sortedFavorites = [...favorites].sort((a, b) => {
    const scoreA = morningScores[a.id] ?? 0;
    const scoreB = morningScores[b.id] ?? 0;
  ...
  if (matchingAlerts.length === 0) {
    ...
    const topAlerts = matchingAlerts.filter((a) =>
      a.affectedLines.some((line) => topLines.has(line.toUpperCase()))
    );
    ...
    // Prioritize top-line alerts in the message
    if (topAlerts.length > 0 && topAlerts.length < matchingAlerts.length) {
      body = `Heads up: Delays on your morning lines ${topLinesList}. Also affected: ${affectedList}`;
    } else {
