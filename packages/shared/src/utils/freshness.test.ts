...
describe("freshness utilities", () => {
  ...
  describe("getFreshnessTextColor", () => {
    it("returns correct Tailwind classes for each level", () => {
      expect(getFreshnessTextColor("fresh")).toBe("text-green-700 dark:text-green-400");
      expect(getFreshnessTextColor("neutral")).toBe(
        "text-text-tertiary dark:text-dark-text-tertiary"
      );
      expect(getFreshnessTextColor("amber")).toBe("text-amber-700 dark:text-amber-400");
      expect(getFreshnessTextColor("red")).toBe("text-red-700 dark:text-red-400");
    });
  });
