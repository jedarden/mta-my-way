...
vi.mock("./ShuttleInfo.jsx", () => ({
  ShuttleInfo: ({ shuttleInfo, compact }: { shuttleInfo: ShuttleBusInfo; compact?: boolean }) => (
    <div data-testid="shuttle-info" data-compact={compact}>
      Shuttle: {shuttleInfo.stops.map((stop) => stop.description).join(" → ")}
    </div>
  ),
}));
...
const mockAlert1 = makeStationAlert({
  id: "alert-1",
  severity: "severe",
  source: "official",
...
const mockAlert2 = makeStationAlert({
  id: "alert-2",
  severity: "warning",
  source: "predicted",
...
const mockAlert3 = makeStationAlert({
  id: "alert-3",
  severity: "info",
  source: "official",
...
const mockAlertWithShuttle = makeStationAlert({
  id: "alert-shuttle",
  severity: "warning",
  source: "official",
  ...
  shuttleInfo: {
    ...
    toStopId: "128",
    stops: [
      { nearStationId: "127", description: "Station A" },
      { nearStationId: "128", description: "Station B" },
    ],
...
describe("AlertBanner", () => {
  describe("basic rendering", () => {
    ...
    it("renders singular '1 Alert' for single alert", () => {
      render(<AlertBanner alerts={[mockAlert1]} />);
  ...
  describe("severity styling", () => {
    it("applies severe styling for severe alerts", () => {
      const { container } = render(<AlertBanner alerts={[mockAlert1]} />);
    ...
    it("applies warning styling for warning alerts", () => {
      const { container } = render(<AlertBanner alerts={[mockAlert2]} />);
    ...
    it("applies info styling for info alerts", () => {
      const { container } = render(<AlertBanner alerts={[mockAlert3]} />);
  ...
  describe("expand/collapse behavior", () => {
    it("shows most severe alert headline when collapsed", () => {
      ...
      const headlines = screen.getAllByText(mockAlert1.headline);
      expect(headlines.length).toBeGreaterThan(0);
      // Second alert should not be visible when collapsed
      expect(screen.queryByText(mockAlert2.headline)).not.toBeInTheDocument();
    });
    ...
    it("expands to show more alerts when clicked", async () => {
      ...
      const allHeadlines = screen.getAllByText(mockAlert2.headline);
      expect(allHeadlines.length).toBeGreaterThan(0);

      const allHeadlines2 = screen.getAllByText(mockAlert3.headline);
  ...
  describe("predicted alerts", () => {
    it("shows '(predicted)' label for predicted alerts", () => {
      render(<AlertBanner alerts={[mockAlert2]} maxVisible={1} />);
...
describe("SingleAlertBanner", () => {
  it("renders single alert", () => {
    render(<SingleAlertBanner alert={mockAlert1} />);

    expect(screen.getByText(mockAlert1.headline)).toBeInTheDocument();
  });

  it("applies severity styling", () => {
    const { container } = render(<SingleAlertBanner alert={mockAlert1} />);
  ...
  it("shows predicted label for predicted alerts", () => {
    render(<SingleAlertBanner alert={mockAlert2} />);
