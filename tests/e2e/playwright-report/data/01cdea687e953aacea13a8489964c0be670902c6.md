...
- Name: trip-tracking.e2e.ts >> Trip Tracking Screen >> Trip Actions >> should have stop tracking button
- Location: trip-tracking.e2e.ts:123:5
...
Locator: locator('role=button:has-text("Stop Tracking")').or(locator('role=button[aria-label*="stop" i]'))
Expected: attached
InvalidSelectorError: Error while parsing selector `button:has-text("Stop Tracking")` - unexpected symbol ":" at position 6
...
Call log:
  ...
  - waiting for locator('role=button:has-text("Stop Tracking")').or(locator('role=button[aria-label*="stop" i]'))
