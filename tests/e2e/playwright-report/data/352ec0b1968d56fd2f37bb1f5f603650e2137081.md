...
- Name: settings.e2e.ts >> Settings Screen >> Settings Loading >> should navigate back when back button is clicked
- Location: settings.e2e.ts:33:5

# Error details

```
Error: locator.click: InvalidSelectorError: Error while parsing selector `link[aria-label*="back" i], role=button[aria-label*="back" i]` - unexpected symbol "," at position 26
