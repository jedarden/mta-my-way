...
- Name: trip-tracking.e2e.ts >> Trip Tracking Screen >> Trip Actions >> should navigate back when stop tracking is clicked
- Location: trip-tracking.e2e.ts:132:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `button:has-text("Stop Tracking")` - unexpected symbol ":" at position 6
