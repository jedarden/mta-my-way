...
- Name: subway-year.e2e.ts >> Stats / Subway Year Screen >> Accessibility >> should have aria-pressed on time window buttons
- Location: subway-year.e2e.ts:566:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `button:has-text("This"), role=button:has-text("All")` - unexpected symbol ":" at position 6
