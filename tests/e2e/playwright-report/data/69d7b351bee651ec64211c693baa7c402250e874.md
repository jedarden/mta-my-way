...
- Name: map.e2e.ts >> Map Screen >> Accessibility >> should have proper ARIA labels on map controls
- Location: map.e2e.ts:331:5
...
Locator: locator('role=button[aria-label*="filter" i], role=button:has-text("Filter")')
Expected: attached
InvalidSelectorError: Error while parsing selector `button[aria-label*="filter" i], role=button:has-text("Filter")` - unexpected symbol "," at position 30
...
Call log:
  ...
  - waiting for locator('role=button[aria-label*="filter" i], role=button:has-text("Filter")')
