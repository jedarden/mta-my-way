...
- Name: trip-tracking.e2e.ts >> Trip Tracking Screen >> Anomaly Detection >> should show anomaly banner for delayed trips
- Location: trip-tracking.e2e.ts:93:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `alert:has-text(/delay|longer than usual/i)` - unexpected symbol ":" at position 5
