...
- Name: subway-year.e2e.ts >> Stats / Subway Year Screen >> Share Functionality >> should show loading state while sharing
- Location: subway-year.e2e.ts:459:5

# Error details

```
Error: locator.click: InvalidSelectorError: Error while parsing selector `button:has-text("Share My Subway Year")` - unexpected symbol ":" at position 6
...
Call log:
  - waiting for locator('role=button:has-text("Share My Subway Year")')
