...
- Name: subway-year.e2e.ts >> Stats / Subway Year Screen >> Navigation >> should navigate to stats screen from journal
- Location: subway-year.e2e.ts:21:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `button:has-text("Subway Year"), role=link[href="/stats"]` - unexpected symbol ":" at position 6
