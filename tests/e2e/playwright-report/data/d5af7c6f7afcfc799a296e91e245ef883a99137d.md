...
- Name: settings.e2e.ts >> Settings Screen >> Notification Settings >> should allow enabling push notifications
- Location: settings.e2e.ts:184:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `button:has-text("Enable")` - unexpected symbol ":" at position 6
