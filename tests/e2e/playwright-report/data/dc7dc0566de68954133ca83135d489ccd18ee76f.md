...
- Name: trip-tracking.e2e.ts >> Trip Tracking Screen >> Trip Journaling Integration >> should link to commute journal
- Location: trip-tracking.e2e.ts:416:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `link:has-text("journal")` - unexpected symbol ":" at position 4
