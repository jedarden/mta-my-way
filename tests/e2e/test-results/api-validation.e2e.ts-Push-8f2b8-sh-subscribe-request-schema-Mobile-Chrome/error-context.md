...
- Name: api-validation.e2e.ts >> Push notification validation >> validates push subscribe request schema
- Location: api-validation.e2e.ts:100:3
...
> 115 |     expect(response.status()).toBe(200);
  ...
  131 |     expect(body).toHaveProperty("error");
  132 |   });
  133 | });
  134 | 
```