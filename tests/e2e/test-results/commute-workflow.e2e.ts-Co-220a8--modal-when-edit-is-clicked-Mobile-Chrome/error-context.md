...
- Name: commute-workflow.e2e.ts >> Commute Workflow >> Commute Editing >> should open edit modal when edit is clicked
- Location: commute-workflow.e2e.ts:392:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `button[aria-label*="edit" i], role=button:has-text("Edit")` - unexpected symbol "," at position 28
