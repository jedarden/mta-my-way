...
- Name: commute-workflow.e2e.ts >> Commute Workflow >> Commute Detail View >> should display commute name and route
- Location: commute-workflow.e2e.ts:257:5
...
Locator: locator('role=heading[name="Work"]')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('role=heading[name="Work"]')
...
```ts
  ...
  199 | 
  200 |         // Try to save (might fail if form is not filled)
  201 |         const saveButton = page.locator('role=button:has-text("Save")');
  202 |         const hasSave = await saveButton.count();
  203 | 
  204 |         if (hasSave > 0) {
  205 |           // Check if save is enabled
  206 |           const isDisabled = await saveButton.isDisabled();
  207 | 
  208 |           if (!isDisabled) {
  209 |             await saveButton.click();
  210 | 
  211 |             // Modal should close
  212 |             const modal = page.locator('[role="dialog"]');
  213 |             await expect(modal).not.toBeVisible({ timeout: 5000 });
  214 |           }
  215 |         }
  216 |       }
  217 |     });
  218 |   });
  219 | 
  220 |   test.describe("Commute Detail View", () => {
  221 |     test.beforeEach(async ({ page }) => {
  222 |       // Set up a test commute
