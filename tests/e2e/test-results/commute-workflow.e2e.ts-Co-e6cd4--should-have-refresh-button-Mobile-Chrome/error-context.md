...
- Name: commute-workflow.e2e.ts >> Commute Workflow >> Commute Analysis >> should have refresh button
- Location: commute-workflow.e2e.ts:360:5
...
Locator: locator('role=button[aria-label*="refresh" i], role=button:has-text("Refresh")')
Expected: attached
InvalidSelectorError: Error while parsing selector `button[aria-label*="refresh" i], role=button:has-text("Refresh")` - unexpected symbol "," at position 31
...
Call log:
  ...
  - waiting for locator('role=button[aria-label*="refresh" i], role=button:has-text("Refresh")')
