...
- Name: fare-tracking.e2e.ts >> Fare Tracking >> Monthly Comparison >> should show comparison values
- Location: fare-tracking.e2e.ts:235:5
...
Locator: locator('text=/Pay-per-ride:\\s*\\$\\d+/i')
Expected: attached
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=/Pay-per-ride:\\s*\\$\\d+/i')
