...
- Name: line-diagram.e2e.ts >> Line Diagram Screen >> Diagram Loading >> should have back button
- Location: line-diagram.e2e.ts:37:5
...
Locator: locator('role=link[aria-label*="back" i], role=button[aria-label*="back" i]').first()
Expected: attached
InvalidSelectorError: Error while parsing selector `link[aria-label*="back" i], role=button[aria-label*="back" i]` - unexpected symbol "," at position 26
...
Call log:
  ...
  - waiting for locator('role=link[aria-label*="back" i], role=button[aria-label*="back" i]').first()
