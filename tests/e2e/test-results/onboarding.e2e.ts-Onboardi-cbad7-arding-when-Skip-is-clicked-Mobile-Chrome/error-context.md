...
- Name: onboarding.e2e.ts >> Onboarding Flow >> Welcome Screen >> should complete onboarding when Skip is clicked
- Location: onboarding.e2e.ts:56:5

# Error details

```
Error: page.click: InvalidSelectorError: Error while parsing selector `button:has-text("Skip")` - unexpected symbol ":" at position 6
...
Call log:
  - waiting for locator('role=button:has-text("Skip")')
