...
- Name: settings.e2e.ts >> Settings Screen >> Display Preferences >> should allow adjusting refresh interval
- Location: settings.e2e.ts:146:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `button:has-text(/\d+ sec/), [role="combobox"], input[type="range"]` - unexpected symbol ":" at position 6
