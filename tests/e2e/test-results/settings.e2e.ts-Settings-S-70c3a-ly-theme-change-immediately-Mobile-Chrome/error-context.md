...
- Name: settings.e2e.ts >> Settings Screen >> Theme Settings >> should apply theme change immediately
- Location: settings.e2e.ts:84:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `button:has-text("Dark")` - unexpected symbol ":" at position 6
