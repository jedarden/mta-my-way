...
- Name: settings.e2e.ts >> Settings Screen >> Data Management >> should export data when requested
- Location: settings.e2e.ts:262:5

# Error details

```
Error: locator.count: InvalidSelectorError: Error while parsing selector `button:has-text("Export")` - unexpected symbol ":" at position 6
