...
- Name: settings.e2e.ts >> Settings Screen >> Navigation >> should navigate to other screens via bottom nav
- Location: settings.e2e.ts:556:5

# Error details

```
Error: locator.count: Unexpected token "=" while parsing css selector "[role="navigation"] role="link"]". Did you mean to CSS.escape it?
```
