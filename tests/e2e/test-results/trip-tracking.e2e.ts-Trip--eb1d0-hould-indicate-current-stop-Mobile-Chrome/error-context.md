...
- Name: trip-tracking.e2e.ts >> Trip Tracking Screen >> Active Trip Display >> should indicate current stop
- Location: trip-tracking.e2e.ts:54:5
...
Locator: locator('[aria-current], [class*="current" i], [class*="active" i]')
Expected: attached
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[aria-current], [class*="current" i], [class*="active" i]')
